...
The system_dev_harness package provides agent prompts, workflow policies, and system-definition context that must be usable across multiple target development repositories. Each target repository should receive a self-contained copy of the runtime context without depending on a shared installation location, network service, or external database. The deployment model needs to be simple, reliable, and inspectable.

## Decision

Package the dev harness as a **copyable portable payload**:

- The runtime context (agent prompts, workflow policies, system-definition guidance, templates) lives under `.opencode/dev_harness/` in this package repository.
...
- The system-definition source documentation (`system_definition/`) stays in this package repository. Target repositories receive only the copied guidance under `.opencode/dev_harness/systems_engineering/`.
...
- `system_definition/cross-cutting/05-operation/deployment-process.md` — Deployment requirements
