...
- `.opencode/dev_harness/systems_engineering/decision-log.md` (entire file: describes product-level decision index only — ID, Title, Layer, Status, Location, Related artifacts — no workflow governance decisions)
- `.opencode/dev_harness/systems_engineering/decision-placement.md` (entire file: placement rules for product decisions — no placement for workflow process decisions)
