...
The current solution is packaged as copyable OpenCode configuration and prompts. Only `opencode.json` and `.opencode/` are copied into a development repo; the `system_definition/` tree stays in this repository as system-definition source documentation and traceability.
...
- `.opencode/dev_harness/systems_engineering/` - reusable product breakdown guidance split into small files for copied target-repo agents.
...
- `system_definition/` - system-definition source documentation and traceability retained in this repository only.
...
| --- | --- | --- |
| Product source rationale, commitments, architecture, decisions, verification expectations, operation requirements, and traceability | `system_definition/` | Product-breakdown updates in guarded delivery or improvement work. |
| Runnable install, deploy, usage, verification, troubleshooting, and contributor procedures | `docs/` | Documentation updates; link to system-definition context instead of copying it. |
...
| Improvement candidates and future work seeds | Accepted candidates in `system_definition/cross-cutting/06-evolution/candidates/`, then selected/done evolution files | `orchestrator-builder` in candidate-capture mode. |
...
| External research claims and source notes | `knowledge/agent-reasoning/` plus cited decisions in `system_definition/` | Research-backed product work. |
...
- `orchestrator-builder` owns implementation, candidate-capture persistence, scoped cleanup, documentation/system-definition updates, build-error resolution, cleanup-helper routing, review-helper routing for builder-owned review pass evidence, and research helper routing. Builder-owned review helper calls use `caller_context: builder_preflight`.
...
The product breakdown guidance is implemented as copied agent context under `.opencode/dev_harness/systems_engineering/`.

| Artifact | Purpose |
| --- | --- |
| `.opencode/dev_harness/systems_engineering/README.md` | Entry point, load-on-demand routing table, recommended layered tree, and layer questions. |
| `.opencode/dev_harness/systems_engineering/layers/*.md` | One small context file per layer. |
| `.opencode/dev_harness/systems_engineering/decision-placement.md` | Rule for placing distributed decisions near affected artifacts. |
| `.opencode/dev_harness/systems_engineering/decision-log.md` | Guidance for maintaining the global decision index. |
| `.opencode/dev_harness/systems_engineering/traceability.md` | Cross-layer traceability chain and checklist. |
| `.opencode/dev_harness/systems_engineering/naming.md` | Stable ID and filename prefixes. |
| `.opencode/dev_harness/systems_engineering/templates/decision-template.md` | Reusable decision record template. |
| `.opencode/dev_harness/systems_engineering/templates/decision-log-entry-template.md` | Reusable compact decision-log entry template. |
| `.opencode/dev_harness/systems_engineering/templates/improvement-backlog-overview-template.md` | Reusable overview template for improvement backlogs. |
| `.opencode/dev_harness/systems_engineering/templates/improvement-candidate-template.md` | Reusable per-candidate improvement backlog template. |
...
| `.opencode/dev_harness/workflow/product-breakdown-work.md` | System-definition loading, placement, traceability, decision, and index evidence rules. |
