# PD-001: System Definition Structure Rationale
...
- **PBS (System Definition Structure)**: Captures physical/structural layers — architecture boundaries, implementation maps, and their respective decisions.
...
Each layer is documented in a dedicated directory under `system_definition/` and maintains its own local `decisions/` directory for layer-specific decision records.
...
- `system_definition/README.md` — describes the decomposition model
- `system_definition/breakdown-structures.md` — formal FBS-PBS-WBS relationship model
- `system_definition/product-tree.md` — hierarchical product tree visualization
- `system_definition/decision-placement.md` — placement rules reference layer model
- All layer README files under `fbs/`, `pbs/`, and `cross-cutting/`

## Verification

The directory structure matches the decomposition model described in system_definition/README.md. Every artifact file exists in a directory matching its layer classification. The product-tree.md Mermaid diagram correctly reflects the on-disk structure.
