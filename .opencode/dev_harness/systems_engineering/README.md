# System Definition Agent Context
...
Use `system_definition/` for product source information: intent, users, scope, capabilities, stable decisions, architecture, implementation mapping, verification expectations, operational product requirements, and evolution direction.
...
Do not duplicate the same example, command, or policy text in both places. If a guide needs product context, link to system-definition artifacts. If a product artifact needs practical steps, state the durable requirement and link to the relevant guide under `docs/`.
...
## Expected System Definition Tree
...
6. Put command examples, install steps, usage guides, and contributor workflow in `docs/`, linking back to system-definition artifacts for context.
