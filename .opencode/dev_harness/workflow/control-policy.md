...
Use `workflow_mode: delivery` when the user asks for actual changes now. Use `workflow_mode: candidate_capture` when the user asks for a proposal, recommendation, evaluation, discovery, documented candidate, future task seed, or backlog item.
